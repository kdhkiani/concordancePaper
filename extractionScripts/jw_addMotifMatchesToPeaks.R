...
if (length(cmdargs) == 0) {
  diffPeakTib    <- read_tsv(here('extractedData','Jurida_Weiterer','ATACseq', 'jw_AtacPeaksMergedSummitWindows_mergedist50.tsv'))
  dpOutputTibLoc <- here('extractedData','Jurida_Weiterer','ATACseq', 'jw_AtacPeaksMergedSummitWindows_mergedist50.annotated.tsv')
...
dpOutputTib <- dpOutputTib %>% 
  tidyr::separate(col = peak, into = c("chrom", "startLocs", "endLocs"), sep = ":") %>% 
  dplyr::mutate(startLocs = as.numeric(startLocs)) %>% 
  dplyr::mutate(endLocs = as.numeric(endLocs)) %>% 
  dplyr::mutate(width = endLocs - startLocs)

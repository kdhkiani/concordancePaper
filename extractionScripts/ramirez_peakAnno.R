...
if (length(cmdargs) == 0) {
  peaksFile     <- here('extractedData', 'Ramirez', 'ATACseq', 'ramirez_AtacPeaksMergedSummitWindows_mergedist50.bed')
  outputLoc     <- here('extractedData', 'Ramirez', 'ATACseq', 'ramirez_peakAnno_AtacPeaksMergedSummitWindows_mergedist50.tsv')
} else {
  peaksFile     <- cmdargs[1]
  outputLoc     <- cmdargs[2]
}

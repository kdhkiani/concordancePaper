...
class ATAC_Sample:
	...
	def __init__(self, sample_name, input_data_dir, output_dir, macs2_window_size, macs2_summit_window_radius):
		...
		self.read_path     = '{0}/{1}'.format(input_data_dir, sample_name + '.fastq')
...
def run_fastQC(s_obj):
	run_command(' '.join(['fastqc', s_obj.read_path, '--outdir', s_obj.output_data_dir]))
...
def filter_alignments(s_obj):
	...
	run_command(' '.join(['samtools', 'view', '-F', '1804', '-u', s_obj.bowtie2_aligned_reads,
	...
	run_command(' '.join(['samtools', 'view', '-q', '20', '-F', '1804', '-u', s_obj.tmp_early_filt_output,
	...
	run_command(' '.join(['samtools', 'view', '-F', '1804', '-b', s_obj.picard_output, '>', s_obj.final_bam_file]))
...
def generate_text_alignment_formats(s_obj):
	...
	with gzip.open(s_obj.final_bedpe_file, 'rt') as f:
		for line in f:
			...
			if r1_strand == '+': #all set to zero since source data is from DNAse
				r1_pos1 += 0
				r1_pos2 += 0
				r2_pos1 -= 0
				r2_pos2 -= 0

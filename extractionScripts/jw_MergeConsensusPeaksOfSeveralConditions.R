...
if (length(cmdargs) == 0) {
  inputConsensusPeaksFolder <-  here('extractedData', 'Jurida_Weiterer', 'ATACseq', 'consensusPeakFiles')
  outputMergedPeaksFile <- sprintf('"%s"', here('extractedData', 'Jurida_Weiterer', 'ATACseq', 'mergedConsensusPeakFiles', 'allCondsMergedSummitWindows.bed'))
} else {
  inputConsensusPeaksFolder <- cmdargs[1]
  outputMergedPeaksFile <- sprintf('"%s"', cmdargs[2])
}

mergeOrder <- c(here('extractedData', 'Jurida_Weiterer', 'ATACseq', 'consensusPeakFiles', 'il1a_1hr.consensusSummits.bed'),
                here('extractedData', 'Jurida_Weiterer', 'ATACseq', 'consensusPeakFiles', 'untreated.consensusSummits.bed'))
